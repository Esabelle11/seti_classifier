...
print(f"Model loaded  Google net cam from {weight_path}")
...
def CAM_ALG(image):
    ...
    features = model.features(image) #torch.Size([1, 256, 6, 6])
    output = model.classifier(torch.nn.functional.adaptive_avg_pool2d(features,(1,1)).flatten())
